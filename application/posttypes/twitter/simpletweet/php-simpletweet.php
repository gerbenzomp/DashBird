...
class SimpleTweet {
  var $cacheDir = 'simpletweet/cache';
  var $cacheTime = 3600; // 1 hour
  var $user = '';
  var $count = 4;
  function __construct($user,$count=4) {
    $this->user = $user;
    $this->count = $count;
  }

  function get($cacheDir) {
    $filename = "{$cacheDir}/.simpletweet-{$this->user}.cache.json";
  ...
  function getTweets($cacheDir) {
    $tweets = $this->get($cacheDir);
    return $tweets;
  }
